export function setupDissectionAnimation() {
  const section = document.querySelector('.NativeDevelopment') as HTMLElement
  const dissection = document.querySelector(
    '.NativeDevelopment .dissection'
  ) as HTMLImageElement
  const images = dissection.children as HTMLCollectionOf<HTMLElement>
  ...
  function onScroll() {
    ...
    for (let i = 0; i < numImages; i++) {
      ...
      images[i].style.opacity = String(imgPercent)
